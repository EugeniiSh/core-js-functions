 ...
 *   getPolynom()      => null
 */
function getPolynom(...cofs) {
  return function poly(x) {
    let result = 0;
    for (let i = 0; i < cofs.length; i += 1) {
      result += cofs[i] * x ** (cofs.length - 1 - i);
    }
    return result;
  };
}
...
function memoize(func) {
  const cath = new Map();
  return function catches(...params) {
    const key = params.join();
    if (cath.has(key)) return cath.get(key);
...
function logger(func, logFunc) {
  return function log(...params) {
    const str = params.map((item) => JSON.stringify(item)).join(',');
...
function partialUsingArguments(fn, ...args1) {
  return function curried(...args) {
    if (args.length + args1.length >= fn.length) {
      return fn(...args1, ...args);
    }
    return function inCarried(...args2) {
